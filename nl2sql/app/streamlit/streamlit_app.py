...
def _build_manager() -> tuple[ConversationManager, str]:
    ...
    for (tbl,) in tables:
        ...
        for col_name, col_type in cols:
            ...
            schema_dict[tbl][col_name] = col_type
        lines.append("")
    schema_text = "\n".join(lines).strip()

    from nl2sql.nlp_sql_postgres_v2 import _SYSTEM_PROMPT as NL2SQL_PROMPT
    llm = OpenAIModel(settings.llm_model, base_url=settings.llm_base_url, api_key=settings.llm_api_key)
    pydantic_agent = Agent(model=llm, result_type=str, system_prompt=NL2SQL_PROMPT)
...
def _render_result_with_reasoning(qr: QueryResult) -> None:
    """Render QueryResult to Streamlit, showing reasoning in an expander if present."""
    if qr.reasoning:
        with st.expander("🧠 Model reasoning", expanded=False):
            st.markdown(qr.reasoning)
    st.markdown(_render_result(qr))
...
def main() -> None:
    ...
    if prompt := st.chat_input("e.g. How many documents are stored?"):
        ...
        with st.chat_message("assistant"):
            ...
            _render_result_with_reasoning(qr)
