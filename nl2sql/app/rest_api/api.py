...
async def _get_manager() -> tuple[ConversationManager, str]:
    ...
    _manager = ConversationManager(
        conn=conn, agent=pydantic_agent, schema_text=_schema_text,
        cache_size=50, max_retries=3, max_result_rows=500, query_timeout=30.0,
    )
    return _manager, _schema_text
...
async def query(request: QueryRequest) -> QueryResponse:
    ...
    try:
        ...
        return QueryResponse(
            question=qr.nl_query, sql=qr.sql, columns=qr.columns,
            rows=[list(row) for row in qr.rows], row_count=len(qr.rows),
            cached=qr.cached, attempts=qr.attempts, error=qr.error, success=qr.success,
        )
    except Exception as exc:
...
async def history() -> list[HistoryEntry]:
    ...
    try:
        manager, _ = await _get_manager()
        return [
            HistoryEntry(question=nl, sql=sql, success=qr.success, row_count=len(qr.rows))
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("nl2sql.app.rest_api.api:app", host="0.0.0.0", port=8001, reload=True)
