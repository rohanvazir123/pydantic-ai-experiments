...
Add auto-scroll to the bottom of the user list when users load. Create `basics/basics/src/components/ScrollList.tsx`:
...
Create `basics/basics/src/context/ThemeContext.tsx`:
...
Wrap `App` with the provider in `basics/basics/src/main.tsx`:
...
Create `basics/basics/src/components/SearchBar.tsx`:
...
Create `basics/basics/src/components/FormField.tsx`:
...
Extract everything fetch-related from `App.tsx` into a reusable hook. Create `basics/basics/src/hooks/useUserSearch.ts`:
