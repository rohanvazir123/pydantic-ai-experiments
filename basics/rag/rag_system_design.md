...
- [Multi-Stage Small Model Architecture](#multi-stage-small-model-architecture)
...
A 3B–8B small model can act as a guard at multiple stages of the RAG pipeline — inserting at each micro-stage to filter noise and protect the main (expensive) LLM without adding significant latency. The pattern uses the same model family across three distinct pipeline positions:
...
STAGE 1 ──► [ Small Model: Guardrails & Intent Routing ]
        ...
        [ Vector DB Retrieval ] ──► Returns Top 10 Chunks
                 │
                 ▼
STAGE 2 ──► [ Small Model: Chunk Relevance Filter ] ──► Drops irrelevant chunks
                 │
                 ▼ (Only 3–4 high-quality chunks remain)
...
**Stage 1 — Pre-retrieval: guardrails and intent routing.** Before hitting the vector DB, the small model evaluates the incoming query for safety (prompt injection, jailbreaks, off-scope overrides) and classifies intent — whether the query actually requires retrieval ("What are our Q3 compliance rules?" → vector DB) or is chit-chat ("Thank you!" → fast direct response, bypassing RAG entirely).

**Stage 2 — Post-retrieval: binary chunk relevance filtering.** Vector databases return noisy top-k chunks that share keywords but may be irrelevant. The small model evaluates each chunk with a YES/NO relevance verdict. Dropping NO chunks before sending to the main LLM shrinks prompt token overhead and eliminates distracting context that degrades generation quality.

**Stage 3 — Post-generation: output auditing.** After the main LLM generates a response, the small model checks: (a) hallucination — is every claim grounded in the retrieved documents? (b) PII leakage — did the LLM accidentally surface API keys, internal IDs, or user emails? (c) structured output validity — does the JSON or Markdown match the expected schema?

**Production efficiency:** Constrain the small model to `max_tokens=2` with structured output (Pydantic, instructor) forcing `True/False` or a single JSON key. This keeps each micro-stage to a few milliseconds. Use speculative decoding if your inference stack supports it.

### Design questions

**61.** Walk through every point in the RAG pipeline where a small model (3B–8B) provides value as a guard or filter. What does it do at each stage, what does it protect against, and what is the latency budget for each call?

**62.** How do you implement binary chunk relevance filtering between retrieval and generation? What does a YES/NO small-model filter add over a cross-encoder reranker, and when would you use one versus the other?

**63.** How do you use a small model for post-generation output auditing — hallucination detection, PII scanning, and structured output validation? What are the failure modes of using a model to judge the output of a similar model, and how do you calibrate it?

Full design, prompt templates, and failure analysis: [.answers/15_small_model_pipeline.md](.answers/15_small_model_pipeline.md)
