...
### 1. End-to-end RAG pipeline architecture and failure detection
...
### 2. RAG vs fine-tuning vs full context — when does each win?
...
### 3. Chunk size — failure modes of too small vs too large, and token cost
...
### 4. Retrieved chunk has the answer but lacks context — parent-child retrieval
...
### 6. Reranking architecture — what a cross-encoder adds and what it costs
...
### 7. Retrieval failure detection — before the LLM hallucinates
...
### 8. The lost-in-the-middle problem and how to mitigate it
...
### 9. Hallucination prevention — layers and what each still fails to prevent
...
### 10. When the context is insufficient — detecting and handling gracefully
...
### 11. Faithfulness, relevance, context recall — what each metric misses
...
### 12. RAGAS — what it is and its specific limitations

> What is RAGAS and what are its specific limitations? When does it give misleading scores?

**Why it's top 20:** RAGAS is widely used but poorly understood. Teams that cite RAGAS scores without understanding its failure modes are making decisions on flawed data.

**What a strong answer covers:**
- LLM judge bias: if you generate with GPT-4 and judge with GPT-4, they share biases
- Faithfulness gaming: short, hedged answers score high on faithfulness but low on usefulness
- Answer relevance as a proxy: measures by generating questions from the answer, not by direct relevance assessment
- Cost: $0.05–0.10/query — only practical on 1–5% of production traffic
...
### 13. Document ingestion pipeline for 500K documents with continuous updates
...
### 14. Document updates without re-indexing the entire corpus
...
### 15. RAG latency components — critical path and token cost trade-offs
...
### 16. What can be cached and what invalidation is required
...
| 2 | RAG vs fine-tuning vs full context | [01](01_pipeline_architecture.md) | Trade-off matrix |
| 3 | Chunk size + token cost | [02](02_chunking_strategy.md) | Precision vs sufficiency vs cost |
| 4 | Parent-child retrieval for context boundaries | [02](02_chunking_strategy.md) | Index small, return large |
| 5 | Dense vs sparse vs hybrid (RRF) | [03](03_embedding_and_indexing.md) | Each fails differently |
| 6 | Cross-encoder reranking | [04](04_retrieval.md) | Interaction modelling |
| 7 | Retrieval failure detection | [04](04_retrieval.md) | Catch before LLM hallucinates |
| 8 | Lost-in-the-middle + book-end ordering | [05](05_context_assembly.md) | Attention is not uniform |
| 9 | Hallucination prevention — 4 layers | [06](06_generation_hallucination.md) | No single technique sufficient |
| 10 | Insufficient context — graceful handling | [06](06_generation_hallucination.md) | Never hallucinate to fill gap |
| 11 | Evaluation metrics + blind spots | [07](07_evaluation.md) | Layered metric |
| 12 | RAGAS limitations | [07](07_evaluation.md) | LLM judge bias |
| 13 | Ingestion pipeline at 500K docs | [08](08_knowledge_base_ingestion.md) | Queue + idempotency + batching |
| 14 | Incremental updates without full re-index | [08](08_knowledge_base_ingestion.md) | Chunk-level atomic swap |
| 15 | Latency components + token cost trade-offs | [09](09_latency_performance.md) | Critical path + parallelism |
| 16 | Caching + invalidation | [09](09_latency_performance.md) | Schema version hash in cache key |
