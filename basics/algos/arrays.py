...
def find_len_longest_common_subsequence(A : list[int], B : list[int]) -> list[list[int]]:
  ...
  # Initializa 2d dynamic programming array 
...
def longest_common_subsequence(A: list[int], B: list[int], L: list[ list[int] ]):
  m, n = len(A), len(B)
  ...
  while i > 0 and j > 0:
    if A[i-1] == B[j-1]:
      lcs[index-1] = A[i-1] # pick the elem
      i -=1
      j -= 1
...
def longest_increasing_subsequence(A: list[int], B: list[int], L: list[ list[int] ]):
  m, n = len(A), len(B)
  i , j = m, n
  index = L[m][n]
  lcs = [None] * (index)
...
def print_longest_increasing_consecutive_subsequence(A : list[int], longest_len :int , max_index: int):
...
def buy_and_sell_stock(stock_prices:List[float]) -> Tuple[float, List[float]]:
  ...
  for price_today in stock_prices:
...
def buy_and_sell_stock_twice(stock_prices:List[float]) -> float:
  ...
  for i, price_today in reversed (list(enumerate(stock_prices))):
    ...
    max_total_profit = max(max_profit_if_bought_today + max_profit_per_day_first_buy_sell[i], max_profit)
    max_profit_per_day_total = max_total_profit
  return max_profit, max_profit_per_day_total
