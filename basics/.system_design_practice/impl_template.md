...
| External data | **SQLModel over an external read-replica engine** + **guarded API clients** (§9) | map read-only external tables (bureau/ledger replicas, partner DBs) to SQLModel models on a *second* async engine; side-effecting integrations go through typed API clients with breakers, not direct DB access |
| DB access | **SQLModel** (Pydantic-native ORM over SQLAlchemy) on **asyncpg** | one model class is both the table and the API-edge schema; drop to SQLAlchemy Core / raw SQL for hot-path queries |
| Migrations | **Alembic** (autogenerate from SQLModel metadata) | forward-only; expand/contract for zero-downtime; review every autogenerated revision |
...
- **Schema source of truth:** **SQLModel** model classes; Alembic autogenerates
  migrations from their metadata, and the same class serves as the API-edge schema.
- **Migrations:** Alembic, forward-only; **expand/contract** for zero-downtime.
- **External data:** map read-only external replicas to SQLModel models on a
  **second async engine**; side-effecting externals stay guarded API clients (§9).
...
One `SQLModel` class is both the ORM table and the Pydantic schema — define the
domain tables once and reuse them at the API edge:
...
# repository — thin async data access
...
**Read-only external data:** map external replicas (bureau / ledger / partner DBs)
to their own SQLModel models bound to a **second async engine** — read them with
the same ORM, no bespoke client for read-heavy paths:

```python
bureau_engine = create_async_engine(BUREAU_REPLICA_URL)   # read-only account
async with AsyncSession(bureau_engine) as s:
    score = await s.get(CreditScore, ssn_hash)
```

**Guidance (be honest about the boundary):**
- **Reads** (domain + external replicas) → SQLModel repositories.
- **Hot-path / analytical queries** → drop to SQLAlchemy Core or raw SQL; SQLModel
  is for clarity, not for every query.
- **Side-effecting externals** (issue refund, billable credit *pull*, KYC) → stay
  **guarded, idempotent API clients + circuit breakers** (§9), never an ORM write
  into someone else's DB.
- **No cross-database joins:** two engines can't join in SQL — join in app code, or
  cache the external read into a local table first.
- Set `statement_timeout` on the external engine; map its account
  least-privilege / read-only; treat external data as untrusted input.
