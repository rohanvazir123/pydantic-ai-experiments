...
- [x] Lifespan: on startup — connect stores, create consumer groups, register hooks, build pipeline
- [x] Lifespan: on shutdown — close all DB pools and Redis
- [x] Middleware stack: CorrelationID → StructuredLog → CORS (RateLimiter in Phase 9)

### 8.2 Request/Response Schemas (`knowledge/api/schemas.py`)

- [x] `ChatRequest`: query, corpus_ids, session_id (required), model_tier, message_history
- [x] `SearchRequest`: query, corpus_ids, k (1-50), search_type, include_graph, metadata_filter
- [x] `IngestRequest`: corpus_id, source_path|url, enable_graph_extraction, mode
- [x] `ScheduledJobRequest`: name, source_type, source_config, corpus_id, cron_expr, mode
- [x] `APIResponse[T]`: request_id, data, error (ErrorDetail), cache_hit
- [x] `ErrorDetail`: code, message, details, retry_after_s, doc_url
- [x] All other schemas: Auth, Search, Ingest, Corpus, Scheduler, Eval, Feedback, Memory, Health

### 8.3 Routes

- [x] **`routes/ingest.py`**: POST /v1/ingest (publish IngestJob), GET status (HGETALL), GET stream (SSE)
- [x] **`routes/search.py`**: POST /v1/search → retriever fast-path
- [x] **`routes/auth.py`**: POST /v1/auth/token, POST /v1/auth/refresh (stub — Phase 9 adds RS256)
- [x] **`routes/chat.py`**: POST /v1/chat (blocking), POST /v1/chat/stream (SSE POST, not GET)
- [x] **`routes/corpus.py`**: GET /v1/corpus, cache/invalidate, GET/POST/DELETE ontology
...
- [x] **`routes/logs.py`**: GET /v1/logs → Redis ring buffer, on-demand filters (level/service/corpus/request_id/limit)
- [x] **`routes/health.py`**: GET /health (component checks), GET /metrics (Prometheus)
- [x] **`routes/memory.py`**: GET/DELETE /v1/conversations, GET/POST/DELETE /v1/memories
- [x] **`routes/evaluate.py`**: POST /v1/evaluate/run, GET /v1/evaluate/run/{id}, GET compare
- [x] **`routes/feedback.py`**: POST /v1/feedback, POST /v1/signals
- [x] **`routes/scheduler.py`**: GET/POST/DELETE /v1/scheduler/jobs, POST run-now
- [x] Write unit tests: `tests/unit/test_api.py` — schemas, quota, timeout (27 tests)
