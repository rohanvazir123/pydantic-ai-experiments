...
- [ ] `ChatRequest`: `query`, `corpus_ids`, `model_tier`, `stream`
...
- [ ] Actions: `sendMessage`, `appendToken`, `setCitations`, `setAbstention`, `setFeedback`, `newConversation`, `loadConversation`
...
- [ ] `useChat.ts` hook — manages `sendMessage`: `POST /v1/chat` or SSE streaming; handles abstention responses
